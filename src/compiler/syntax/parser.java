...
/** CUP v0.11a beta 20060608 generated parser.
  * @version Sun Jun 12 18:53:58 CEST 2022
  */
...
class CUP$parser$actions {
	...
	// Aqui se podrian incluir metodos creados por mi. Seria buena idea hacerlo para refactorizar codigo y dejarlo mas presentable.
  ...
  public final java_cup.runtime.Symbol CUP$parser$do_action(
	...
	if(tipoFuncion instanceof TypeFunction){
		...
		ParametroEscribir p = (ParametroEscribir)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-2)).value;
	...
	RESULT = sent;
		...
		Sentencia ls = (Sentencia)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-4)).value;
	...
	//Codigo intermedio
	LabelFactory lf = new LabelFactory();
	LabelIF etiquetaComienzoPara = lf.create(); //etiqueta para el comienzo del bucle para
	LabelIF etiquetaFinPara = lf.create(); //etiqueta para el final del bucle para
	...
	TemporalIF e1Temp = e1.getTemporal(); //temporal del limite inferior
	TemporalIF e2Temp = e2.getTemporal(); //temporal del limite superior
	TemporalIF condicionTemp = tF.create(); //temporal de la condicion
	cb.addQuadruples(e1.getIntermediateCode()); //añadimos las cuadruplas de e1
	cb.addQuadruples(e2.getIntermediateCode()); //añadimos las cuadruplas de e2
	cb.addQuadruple("INL", etiquetaComienzoPara); //añadimos la etiqueta comienzo
	cb.addQuadruple("GR", condicionTemp, e1Temp, e2Temp); // indice > limite superior
	cb.addQuadruple("BRT", condicionTemp, etiquetaFinPara); // si indice > limite superior, vamos al final del bucle
	...
	cb.addQuadruple("MVP", var, e1Temp); 
	cb.addQuadruples(ls.getIntermediateCode()); //añadimos las cuadruplas del cuerpo del bucle para
	cb.addQuadruple("INC", e1Temp, e1Temp); //incrementamos el indice
	cb.addQuadruple("BR", etiquetaComienzoPara); //saltamos al inicio del bucle
	cb.addQuadruple("INL", etiquetaFinPara); //añado etiqueta fin

	Sentencia sent = new Sentencia();
	sent.setIntermediateCode(cb.create());
	System.out.println("Codigo intermedio: " + sent.getIntermediateCode());
	RESULT = sent;
	...
	RESULT = sent;
		...
		Sentencia ls = (Sentencia)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-5)).value;
		int rileft = ((java_cup.runtime.Symbol)CUP$parser$stack.elementAt(CUP$parser$top-4)).left;
		int riright = ((java_cup.runtime.Symbol)CUP$parser$stack.elementAt(CUP$parser$top-4)).right;
		Sentencia ri = (Sentencia)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-4)).value;
		...
		Sentencia ri = (Sentencia)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
	...
	// Codigo intermedio
	LabelFactory lf = new LabelFactory();
	LabelIF etiquetaFinSi = lf.create(); //etiqueta para el final de las sentencias if-else
	LabelIF etiquetaSino = lf.create(); //etiqueta para el comienzo de la parte else
	TemporalIF expTemp = ex.getTemporal(); //temporal de la expresion
	IntermediateCodeBuilder cb = new IntermediateCodeBuilder(scopeManager.getCurrentScope());
	cb.addQuadruples(ex.getIntermediateCode()); //añadimos las cuadruplas de la expresion
	cb.addQuadruple("BRF", expTemp, etiquetaSino); //si no se cumple saltamos a la etiqueta SI
	cb.addQuadruples(ls.getIntermediateCode()); //añadimos las cuadruplas del SI
	cb.addQuadruple("BR", etiquetaFinSi); //saltamos al final del SI-SINO
	cb.addQuadruple("INL", etiquetaSino); //emplazamos la etiqueta
	cb.addQuadruples(ri.getIntermediateCode()); //añadimos las cuadruplas del rellenoif
	cb.addQuadruple("INL", etiquetaFinSi); //emplazamos la etiqueta
	...
	RESULT = s;
		...
		int s2listleft = ((java_cup.runtime.Symbol)CUP$parser$stack.peek()).left;
		int s2listright = ((java_cup.runtime.Symbol)CUP$parser$stack.peek()).right;
		Sentencia s2list = (Sentencia)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
	...
	RESULT = s;
		...
		Llamadafuncion lf = (Llamadafuncion)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-1)).value;
	...
	RESULT = e;
		...
		Token as = (Token)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-2)).value;
		int exprleft = ((java_cup.runtime.Symbol)CUP$parser$stack.elementAt(CUP$parser$top-1)).left;
		int exprright = ((java_cup.runtime.Symbol)CUP$parser$stack.elementAt(CUP$parser$top-1)).right;
		Expresion expr = (Expresion)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-1)).value;
	...
	// Comprobamos si el identificador esta previamente incluido en la tabla de simbolos
	if(!scopeManager.containsSymbol(id.getLexema().toUpperCase())){
	...
	TemporalIF temp = tf.create();
	TemporalIF temp1 = tf.create(); // Direccion base
	TemporalIF temp2 = tf.create();
	TemporalIF tempPosicion = vr.getTemporal(); 
	TemporalIF desplaz = tf.create(); // Se trata del desplazamiento que usaremos para cambiar de posicion y acceder diferentes campos del vector
	SymbolIF simboloVector = scopeManager.searchSymbol(id.getLexema().toUpperCase()); // Obtenemos el simbolo del vector
	TypeIF tVec = scopeManager.searchType(simboloVector.getType().getName());
	int rangoInferiorVector = ((TypeArray)tVec).getRangoInferior(); // Obtenemos el rango inferior del vector
	...
	cb.addQuadruple("SUB", desplaz, tempPosicion, rangoInferiorVector); //Correccion para cuando los rangos de los vectores no empiezan en 0. Restamos a la posicion actual el rango inferior
	cb.addQuadruple("SUB", temp2, temp1, desplaz); //Restamos el desplazamiento a la direccion base para acceder a la posicion que queremos
	...
	// Codigo intermedio
	TemporalFactory tf = new TemporalFactory(scopeManager.getCurrentScope());
	TemporalIF tempAnterior = mv.getTemporal();
	IntermediateCodeBuilder cb = new IntermediateCodeBuilder(scopeManager.getCurrentScope());
	cb.addQuadruples(mv.getIntermediateCode());
	cb.addQuadruple("STPINVERT", tempAnterior);
	expr.setTemporal(tempAnterior);
	expr.setIntermediateCode(cb.create());
	...
	if(simbolo instanceof SymbolConstant){
		if(((SymbolConstant)simbolo).getEsBooleano()){ // es booleano
			if(((SymbolConstant)simbolo).isBool_value()){ // si es true ponemos 1
				Value valor = new Value(1);
				cb.addQuadruple("MV", temp, valor);
			}else{ // si es false ponemos 0
				Value valor = new Value(0);
				cb.addQuadruple("MV", temp, valor);
			}
		} else { // es numero
			Value valor = new Value(((SymbolConstant)simbolo).getNum_value()); //hemen y si es booleano?
			cb.addQuadruple("MV", temp, valor);
		}
	...
	RESULT = expr;
		...
		int idright = ((java_cup.runtime.Symbol)CUP$parser$stack.elementAt(CUP$parser$top-10)).right;
		Token id = (Token)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-10)).value;
		int listaSleft = ((java_cup.runtime.Symbol)CUP$parser$stack.elementAt(CUP$parser$top-2)).left;
		int listaSright = ((java_cup.runtime.Symbol)CUP$parser$stack.elementAt(CUP$parser$top-2)).right;
		Sentencia listaS = (Sentencia)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-2)).value;
	...
	scope.getTypeTable().addType("ENTERO",tsEntero);
	TypeSimple tsBooleano = new TypeSimple(scope, "BOOLEANO");
	scope.getTypeTable().addType("BOOLEANO",tsBooleano);
	...
	// Lanzamos un error si el id esta anteriormente declarado bien en la tabla de simbolos o bien en la tabla de tipos
	if(scopeManager.getCurrentScope().getSymbolTable().containsSymbol(id.getLexema().toUpperCase())){
		semanticErrorManager.semanticFatalError("Simbolo " + id.getLexema().toUpperCase() + " en la linea " + id.getLine() + " declarado anteriormente, error.");
	}
	if(scopeManager.containsType(id.getLexema().toUpperCase())){
		semanticErrorManager.semanticFatalError("Identificador " + id.getLexema().toUpperCase() + " en la linea " + id.getLine() + " declarado anteriormente, error.");
	}
	...
	if(listaVar != null){
		...
		for(Selectorvalororeferencia variable : listaAux){
			// Comprobamos si la tabla de simbolos contiene el simbolo
			if(scopeManager.getCurrentScope().getSymbolTable().containsSymbol(variable.getId())){
				semanticErrorManager.semanticFatalError("Simbolo " + variable.getId() + " en la linea " + id.getLine() + " declarado anteriormente, error.");
			}
	...
	// Comprobamos si el id esta anteriormente declarado bien en la tabla de simbolos o bien en la tabla de tipos
	if(scopeManager.getCurrentScope().getSymbolTable().containsSymbol(id.getLexema().toUpperCase())){
		semanticErrorManager.semanticFatalError("Simbolo " + id.getLexema().toUpperCase() + " en la linea " + id.getLine() + " declarado anteriormente, error.");
	}
	if(scopeManager.containsType(id.getLexema().toUpperCase())){
	...
	if(listaVar != null){
		...
		// Recorremos la lista e introducimos las variables en la tabla de simbolos
		for(Selectorvalororeferencia variable : listaAux){
			// Error si existe simbolo
			if(scopeManager.getCurrentScope().getSymbolTable().containsSymbol(variable.getId())){ //TODO aqui tambien deberia de incluir un error si existe tipo? revisar (y en funcion tambien)
				semanticErrorManager.semanticFatalError("Simbolo " + variable.getId() + " en la linea " + id.getLine() + " declarado anteriormente, error.");
			}
	...
	RESULT = t;
		...
		Token dc = (Token)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-1)).value;
	...
	for(int x = 0; x < listaAux.size(); x++){
		...
		if(variable.getId() == null){ // Si es null, no es un IDEN por lo que lanzamos error
			...
			if(variable.isPorValor()){ //comprobamos que es por valor				
				// Error si existe en la tabla de simbolos
				if(scopeManager.getCurrentScope().getSymbolTable().containsSymbol(variable.getId())){
					semanticErrorManager.semanticFatalError("Simbolo " + variable.getId() + " en la linea " + dc.getLine() + " declarado anteriormente, error.");
				}
				// Error si existe en la tabla de tipos
				if(scopeManager.containsType(variable.getId())){
					semanticErrorManager.semanticFatalError("Identificador " + variable.getId() + " en la linea " + dc.getLine() + " declarado anteriormente, error.");
				}
	...
	// Codigo intermedio
	TemporalIF temp = m.getTemporal();
	IntermediateCodeBuilder cb = new IntermediateCodeBuilder(scopeManager.getCurrentScope());
	cb.addQuadruples(m.getIntermediateCode());
	cb.addQuadruple("STPINVERT", temp);
	vr.setTemporal(temp);
	...
	if(valor.esNumero()){ // si es numero lo aniadimos como numero
		...
		Sentencia def = (Sentencia)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
	...
	// Asignacion de posiciones de memoria para las variables globales y los temporales
	List<ScopeIF> scopes = scopeManager.getAllScopes();
	for(ScopeIF scope: scopes){ // para cada ambito
		int direccionEstatica = 65535;

		// Para simbolos
		List<SymbolIF> simbolos = scope.getSymbolTable().getSymbols(); //traemos la lista de simbolos
		for(SymbolIF simbolo: simbolos){ // para cada simbolo
			if(simbolo instanceof SymbolVariable){ // comprobamos si es una variable
				//Guardamos la direccion de la variable en SymbolVariable
				((SymbolVariable)simbolo).setAddress(direccionEstatica);
				//Actualizamos el valor de direccionEstatica para el proximo simbolo
				direccionEstatica = direccionEstatica - simbolo.getType().getSize();
			}
		}

		// Para temporales
		List<TemporalIF> temporales = scope.getTemporalTable().getTemporals(); //traemos la lista de los temporales
		for(TemporalIF t: temporales){ // para cada temporal
			if (t instanceof Temporal){ // comprobamos si es un temporal
				//Guardamos la direccion del temporal en Temporal
				((Temporal)t).setAddress(direccionEstatica);
				//Actualizamos el valro de direccionEstatica pra el proximo temporal
				direccionEstatica = direccionEstatica - ((Temporal)t).getSize();
			}
		}
	} // Fin de la asignacion de posiciones de memoria para las variables globales y los temporales

	// Generacion de codigo intermedio para iniciar el programa principal e inicializar las variables globales
	for(ScopeIF scope : scopes){ // para cada ambito
		IntermediateCodeBuilder cb = new IntermediateCodeBuilder(scope);

		//Solo para el scope de nivel 0
		if(scope.getLevel() == 0){ 
			List<SymbolIF> simbolos = scope.getSymbolTable().getSymbols(); //traemos la lista de simbolos
			for(SymbolIF simbolo: simbolos){ // para cada simbolo
				if(simbolo instanceof SymbolVariable){ // comprobamos si es una variable
